...
// LikedArtifacts.jsx (Enhanced Premium Version)
...
// import { Helmet } from "react-helmet-async";
// import { Spinner } from "flowbite-react";
// import ArtifactCard from "./ArtifactCard"; // Make sure to import your ArtifactCard component
...
const LikedArtifacts = () => {
  ...
  const fetchLikedArtifacts = useCallback(async () => {
    if (!user) return;
    ...
    try {
      ...
      console.error("Error fetching liked artifacts:", err);
      setError({
        message: err.response?.data?.message || err.message || "Failed to load artifacts",
    ...
    return (
      ...
      >
        <div className="max-w-md">
          <img
            src="https://i.ibb.co/0jq7Z0J/error.png"
            alt="Error occurred"
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={handleRetry}
            className="px-6 py-2 bg-gradient-to-r from-red-500 to-orange-500 text-white rounded-lg shadow hover:shadow-md transition-all"
          >
            Retry Loading
          </motion.button>
        </div>
  ...
  return (
    ...
    >
      {/* <Helmet>
        <title>Liked Artifacts | Artifact Tracker</title>
        <meta name="description" content="View your collection of liked artifacts" />
      </Helmet> */}
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
          {likedArtifacts.length > 0 
            ? `You have ${likedArtifacts.length} saved artifact${likedArtifacts.length !== 1 ? 's' : ''}`
            : "Your collection of liked artifacts will appear here"}
        </p>
      </div>
